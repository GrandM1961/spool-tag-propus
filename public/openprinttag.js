const OpenPrintTag = {
  MATERIAL_TYPES: {
    'pla': 0, 'petg': 1, 'abs': 2, 'asa': 3, 'tpu': 4,
    'pa': 5, 'pa12': 6, 'pc': 7, 'peek': 8, 'pva': 9,
    'hips': 10, 'asa-cf': 11, 'pctg': 12, 'tpu-ams': 13,
    'pa-cf': 14, 'pa-gf': 15, 'pa6-cf': 16, 'pla-cf': 17,
    'pet-cf': 18, 'petg-cf': 19, 'pla-aero': 20, 'pps': 21,
    'pps-cf': 22, 'ppa-cf': 23, 'ppa-gf': 24, 'abs-gf': 25,
    'asa-aero': 26, 'pe': 27, 'pp': 28, 'eva': 29, 'pha': 30,
    'bvoh': 31, 'pe-cf': 32, 'pp-cf': 33, 'pp-gf': 34
  },
  ...
  generateData: function(formData) {
    ...
    const tags = [];
    if (formData.matteFinish) tags.push(16);
    if (formData.silkFinish) tags.push(17);
    if (formData.translucent) tags.push(19);
    if (formData.transparent) tags.push(20);
    if (formData.glitter) tags.push(23);
    if (formData.gradualColorChange) tags.push(28);
    if (formData.coextruded) tags.push(29);
    if (tags.length > 0) {
      data.set(28, tags);
    }
  ...
  parseData: function(cborBytes) {
    ...
    if (decoded.has(10)) result.materialName = decoded.get(10);
    if (decoded.has(11)) result.brand = decoded.get(11);
    if (decoded.has(4)) result.gtin = decoded.get(4).toString();
    ...
    if (decoded.has(16)) result.nominalWeight = decoded.get(16).toString();
    if (decoded.has(17)) result.actualWeight = decoded.get(17).toString();
    if (decoded.has(18)) result.emptySpoolWeight = decoded.get(18).toString();

    if (decoded.has(19)) {
      const colorHex = this.decodeColorBytes(decoded.get(19));
      if (colorHex) result.colorHex = colorHex;
    }

    if (decoded.has(20)) {
      const colorHex2 = this.decodeColorBytes(decoded.get(20));
      if (colorHex2) result.colorHex2 = colorHex2;
    }

    if (decoded.has(21)) {
      const colorHex3 = this.decodeColorBytes(decoded.get(21));
      if (colorHex3) result.colorHex3 = colorHex3;
    }

    if (decoded.has(22)) {
      const colorHex4 = this.decodeColorBytes(decoded.get(22));
      if (colorHex4) result.colorHex4 = colorHex4;
    }
    ...
    if (decoded.has(29)) result.density = decoded.get(29).toString();
    if (decoded.has(30)) result.filamentDiameter = decoded.get(30).toString();
    if (decoded.has(34)) result.minTemp = decoded.get(34).toString();
    if (decoded.has(35)) result.maxTemp = decoded.get(35).toString();
    if (decoded.has(36)) result.preheatTemp = decoded.get(36).toString();
    if (decoded.has(37)) result.bedTempMin = decoded.get(37).toString();
    if (decoded.has(38)) result.bedTempMax = decoded.get(38).toString();
    if (decoded.has(52)) result.materialAbbreviation = decoded.get(52);
    if (decoded.has(55)) result.countryOfOrigin = decoded.get(55);
